...
import { editActivity, updateActivity } from '@/api/activity'
export default {
  data() {
    ...
    return {
      ...
      form:{
        ...
        rules: {
          ...
          time: [
            { required:true,message: '请填写活动时间', trigger: 'change' }
          ],
          price: [
            { required:true,message: '请填写活动价格', trigger: 'blur' },
  ...
  methods: {
    ...
    onSubmit(){
      ...
      updateActivity(this.form.id,this.form).then(response => {
          this.$message({
            message: '更新成功',
            type: 'success'
          })
